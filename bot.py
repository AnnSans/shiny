...
def get_schedule(message):
    ...
    if whitespaces != 2: #бот игнорит ошибку и не крашится
        return None
    day, week, group = message.text.split()
    if is_valid(group):
        web_page = get_page(group, week) #обращаемся к расписанию, просим день, если в этот день нет занятий
        group_schedule = parse_schedule_for_anyday(web_page, day) #то Гуляй, Вася
        if group_schedule == 0:
            resp = '<b>Гуляй, Вася</b>'
        else:
        ...
        resp = 'Такая группа есть разве что в Хогвартсе'
...
def get_tommorow(message):
    ...
    if is_valid(group):
        ...
        if group_schedule == 0:
            resp = '<b>Поздравляю! В этот день нет занятий!</b>'
        else:
            times_lst, notes_lst, locations_lst, lessons_lst,\
                    classrooms_lst = group_schedule
            resp = ''
...
def get_all_schedule(message):
    ...
    days_of_week = ['/monday',
                    ...
                    '/friday',
                    '/saturday']
    translation = ['Понедельник - зайка-рукодельник',
                   'Вторник - соловей-задорник',
                   'Среда - лисичкина еда',
                   'Четверг - волк глазами сверк!',
                   'Пятница - колобком прокатится',
                   'Суббота - баня у енота']
